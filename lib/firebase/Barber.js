...
export const bookBarber = async (id, token) => {
  try {
    const res = await axios.post(
      `https://us-central1-bento-5ad4e.cloudfunctions.net/app/api/barber/book/${id}`,
      token
    );
    return res.data;
  } catch (err) {
...
export const deleteBarberBooking = async (bookingId) => {
  try {
    await axios.delete(
      `https://us-central1-bento-5ad4e.cloudfunctions.net/app/api/barber/delete/${bookingId}`
    );
  } catch (err) {
    throw err;
  }
