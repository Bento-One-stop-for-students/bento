...
export const getAllFoodItems = async () => {
  try {
    ...
    const foodItems = items.docs.map(item => ({id:item.id,...item.data()}));
    return foodItems;
  }catch(err) {
    throw err;
  }
...
export const getUserOrders = async (id) => {
  try {
    const allUserOrder = await database.collection(`users/${id}/orders`).orderBy('timestamp',"desc").get();
    const allOrder = allUserOrder.docs.map(order => ({orderId: order.id,...order.data()}))
    return allOrder
  }catch(err) {
    throw err;
  }
...
export const cancelUserOrder = async (userId, orderId) => {
  try {
    const delete1 = database.doc(`users/${userId}/orders/${orderId}`).delete();
