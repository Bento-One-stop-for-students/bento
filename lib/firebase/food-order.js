...
export const createUserOrder = async (data) => {
  try {
    ...
    const res = await axios.post(
      `https://us-central1-bento-5ad4e.cloudfunctions.net/app/api/snackmen/order/${id}`,
      { ...rest }
    );
    return res.data;
  } catch (err) {
...
export const cancelUserOrder = async (userId, orderId) => {
  try {
    await axios.delete(
      `https://us-central1-bento-5ad4e.cloudfunctions.net/app/api/snackmen/order/delete?userId=${userId}&orderId=${orderId}`
    );
  } catch (err) {
    throw err;
  }
